...
async def updater(c: TelePyroBot, m: Message):
    ...
    try:
        ...
        commit_link = f"<a href='https://github.com/sppidy/pyrosecurity/commit/{commit_id}'>{commit_id[:7]}</a>"
    except:
        commit_link = "None"
    ...
    if not changelog and force_update == False:
        await umsg.edit("`Master I Am Already Up-To-Date!!`")
        return
    ...
    await c.send_message(
        PRIVATE_GROUP_ID,
        f"#UPDATE\n\n**__PyroBot Update__** {commit_link}\n\n**Changelog:**\n{changelog}",
